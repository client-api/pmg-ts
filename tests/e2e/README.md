...
Proxmox Mail Gateway server. Anything you put here:
...
2. (Optional) Add `.github/workflows/e2e.yml` to run them in CI against
   a Proxmox Mail Gateway test instance — the sync pipeline preserves
...
PMG_TOKEN='PMGAPIToken=root@pam!auto=<your-uuid>'
PMG_INSECURE_TLS=1   # only when targeting a self-signed dev server
```
